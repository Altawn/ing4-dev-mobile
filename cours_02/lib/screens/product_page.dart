...
class ProductPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => ProduitNotifier(),
      child: Scaffold(
        body: Consumer<ProduitNotifier>(
          builder: (context, notifier, child) {
            final produit = notifier.getProduit();

            return produit == null
                ? const LoadingView()
                : ProductSuccessView(produit: produit);
          },
        ),
      ),
    );
...
class Scores extends StatelessWidget {
  const Scores({super.key, required this.produit});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
          ...
          child: Row(
            ...
            children: [
              Expanded(
                flex: 44,
                child: _Nutriscore(
                  nutriscore: produit.nutriScore ?? ProductNutriScore.unknown,
                ),
              ),
              VerticalDivider(),
              Expanded(
                flex: 56,
                child: _NovaGroup(
                  novaScore: produit.novaScore ?? ProductNovaScore.unknown,
                ),
              ),
            ],
          ),
        ...
        Divider(),
        _GreenScore(
          greenScore: produit.greenScore ?? ProductGreenScore.unknown,
        ),
      ],
    );
  }
...
class ProductSuccessView extends StatelessWidget {
  const ProductSuccessView({super.key, required this.produit});
          ...
          PositionedDirectional(
            ...
            child: Container(
              ...
              child: Column(
                crossAxisAlignment: .start,
                children: [
                  Text(produit.name ?? '', style: context.theme.title1),
                  Text(produit.brands!.join(', '), style: context.theme.title2),
                  Scores(produit: produit),
                ],
              ),
            ),
          ),
